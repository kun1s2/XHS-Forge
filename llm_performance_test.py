...
# 加载 AI_Frontend_IDE/.env（LLM_API_KEY, LLM_BASE_URL, LLM_MODEL）
_env_path = Path(__file__).resolve().parent / "AI_Frontend_IDE" / ".env"
load_dotenv(dotenv_path=_env_path)

# --- 1. 配置参数（全部使用 .env 的 LLM_*）---
LLM_API_KEY = os.environ.get("LLM_API_KEY", "")
LLM_BASE_URL = os.environ.get("LLM_BASE_URL", "")
LLM_MODEL = os.environ.get("LLM_MODEL", "")

# --- 2. 结构化输出模型 (DSL 宪法：加入物理字数锁) ---
...
# --- 4. 构造标准对话模板 (System + Human) ---
# 💡 System Prompt: 负责确立人设、规则、红线与物理约束
...
async def run_test(name, llm):
    ...
    # 🌟 使用 LCEL 语法，将 prompt_template 管道连接给 LLM
    base_chain = prompt_template | llm


    start = time.perf_counter()
    try:
        if "ChatTongyi" in str(type(llm)):
            # 原生阿里包：直接绑定 Schema
            structured_llm = llm.with_structured_output(MockDSL)
        else:
            # OpenAI / 兼容包：降级使用 function_calling
            structured_llm = llm.with_structured_output(MockDSL)
    ...
    start = time.perf_counter()
    try:
        resp = await base_chain.ainvoke({})  # 传入空字典即可，因为 prompt 里没有挖空变量
    ...
    # 场景 3: 结构化输出 (with_structured_output)
...
async def main():
    test_tasks = [
        ("ChatOpenAI 兼容", llm_openai),
        ("ChatTongyi 原生", llm_tongyi),
    ]
    
    all_res = {}
    for name, llm in test_tasks:
