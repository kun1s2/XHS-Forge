...
更重的语义命中、评分和规则细节下沉到 `composition_support.py`，
...
from app.agents.services.composition_prompts import (
    ...
    build_composition_prompt as _prompt_build_composition_prompt,
    build_composition_prompt_snapshot as _prompt_build_composition_prompt_snapshot,
...
class CompositionAgentState(TypedDict):
    """内容编辑服务在提示词与结构化输出阶段使用的最小状态。"""
...
def _select_composition_tools(selected_element_id: str | None):
    """按编辑范围选择 composition service 可用工具集。"""
    return LOCAL_COMPOSITION_TOOLS if _has_local_selection(selected_element_id) else COMPOSITION_TOOLS


def _build_composition_prompt_snapshot(mode: str, prompt_text: str, plan: Any | None = None) -> dict[str, Any]:
    """构造编辑节点在 Prompt Lab 中展示的快照。"""
    return _prompt_build_composition_prompt_snapshot(mode, prompt_text, plan)
...
def _build_composition_prompt(state: CompositionAgentState) -> str:
    """构造通用编辑提示词。"""
    return _prompt_build_composition_prompt(state)


def _build_local_edit_prompt(state: CompositionAgentState, user_query: str) -> str:
    """构造局部编辑模式的提示词。"""
...
def _default_canvas_block_intents(state: RuntimeState) -> list[dict[str, Any]]:
    """在缺少 planner block intents 时给出一套保守默认结构。"""
    return _prompt_default_canvas_block_intents(state)


def _build_canvas_creation_prompt(state: CompositionAgentState, user_query: str) -> str:
    """构造新建整页画布的提示词。"""
...
def _build_canvas_creation_fallback(state: RuntimeState, user_query: str) -> CanvasCreationOutput:
...
def _build_global_edit_prompt(state: CompositionAgentState, user_query: str) -> str:
    """构造整页编辑模式的提示词。"""
...
async def composition_service(state: RuntimeState) -> dict:
    ...
    if local_mode and target_exists:
        try:
            ...
            return {
                ...
                "turn_trace": {"composition_worker": {"mode": "local", "action": "error", "structured": True, "fallback_used": False, "selected_element_id": selected_element_id, "error": str(e)}},
                "agent_backends": {"composition_worker": "structured_function_calling"},
            }
    ...
    try:
        ...
        return {
            ...
            "turn_trace": {"composition_worker": {"mode": "global", "action": plan.action, "reason": plan.reason, "structured": True, "fallback_used": False, "selected_element_id": selected_element_id, "target_block_id": plan.block_id, "target_block_index": plan.block_index, "new_component_type": plan.new_component_type, "paragraph_index": plan.paragraph_index}},
            "agent_backends": {"composition_worker": "structured_function_calling"},
        }
    except Exception as e:
