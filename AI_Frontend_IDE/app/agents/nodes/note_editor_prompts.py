"""Prompt/context builders for Note Editor.

Keeping prompt assembly here helps `note_editor_node.py` stay focused on the
...
class NoteEditorPromptState(TypedDict):
...
def build_note_editor_prompt_snapshot(mode: str, prompt_text: str, plan: Any | None = None) -> dict[str, Any]:
    plan_payload = {}
    if plan is not None:
    ...
    return build_prompt_snapshot(
        "note_editor",
        system_prompt=f"Structured note_editor ({mode}) prompt. 优先走 NoteDocument + component_manifest + planner_policy，不直接依赖开放式 fallback。",
...
def build_note_editor_prompt(state: NoteEditorPromptState) -> str:
...
def build_local_edit_prompt(state: NoteEditorPromptState, user_query: str) -> str:
    _note_document, document_view, block_style_map, image_assets = build_note_document_editing_context(state)
    selected_element_id = state.get("selected_element_id")
    knowledge = state.get("retrieved_knowledge", {}) or {}
    target_block = next(
...
def build_canvas_creation_prompt(state: NoteEditorPromptState, user_query: str) -> str:
    knowledge = state.get("retrieved_knowledge", {}) or {}
    planner_output = state.get("planner_output", {}) or {}
    planner_policy = state.get("planner_policy", {}) or {}
...
def build_global_edit_prompt(state: NoteEditorPromptState, user_query: str) -> str:
    _note_document, document_view, block_style_map, image_assets = build_note_document_editing_context(state)
    knowledge = state.get("retrieved_knowledge", {}) or {}
    component_contract_text = _build_component_contract_text()
