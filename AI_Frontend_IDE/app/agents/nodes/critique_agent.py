...
class NoteCritiqueAgent:
    ...
    SYSTEM_PROMPT = """你是一位内容工作台里的最终质量复盘助手。你的目标不是挑刺式审稿，而是判断这页内容是否：
1. 真正理解了用户意图
2. 结构与当前场景匹配
3. 表达足够可信，不会把不确定内容包装成真事实
4. 信息是否完整到足以继续展示
5. 页面是否像成熟产品结果，而不是草率 demo

请按以下原则评估：
- 标题/开场是否快速进入主题，但不要为了“标题党”牺牲准确性
- emoji、情绪和 CTA 只算次级观察项，不要把它们当成高权重标准
- factual_issues 只写真正会削弱可信度的事实风险
- completeness_issues 只写真正阻碍用户理解或判断的信息缺口
- suggestions 必须是可执行的改进建议，优先围绕：结构、事实、完整度、用户理解
...
严格要求：
- 不要为了“更像小红书”鼓励虚张声势
- 不要把强 CTA、强情绪、emoji 稠密度当成核心质量标准
- 如果页面已经可信、清楚、结构合理，可以给高分，即使它不夸张"""

    def __init__(self):
        self.llm = create_llm(temperature=0.3)
    
    async def critique(self, state: UIProjectState) -> Dict[str, Any]:
...
async def critique_node(state: UIProjectState) -> Dict[str, Any]:
    """Node wrapper for the critique agent."""
    agent = get_critique_agent()
    return await agent.critique(state)


# Backward-compatible alias for earlier experimental imports.
gaicritique_node = critique_node
